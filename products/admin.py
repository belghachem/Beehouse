...
class ProductStatisticsAdmin(admin.ModelAdmin):
    """
    This is a read-only admin interface to view product statistics.
    Register it separately if you want a dedicated statistics view.
    """
    ...
    def get_queryset(self, request):
        ...
        queryset = queryset.annotate(
            total_ordered=Sum('orderitem__quantity')
        ).order_by('-total_ordered')  # Order by most ordered
        return queryset
